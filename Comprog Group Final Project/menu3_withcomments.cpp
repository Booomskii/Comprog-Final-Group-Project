#include <iostream>
#include <string>
#include <windows.h> // extra2
using namespace std;

int main()
{
    string regusername;		// reg* for registering account for new users
    string regfname;
    string reglname;
    string regpass;
    string regeaddress;
    ...
    string regaddress2;
    string regnum;
    string defuser = "dev";		// dev* for existing account
    ...
    string loguser;
    string logpass;
    int command;	// mainpage nga input select ug number
    int exit;	// exit nga input select ug number
    int a = 0;		// if condition kung mopili ug 'ONLINE payment'
    int b = 0;		// if condition kung mopili ug 'BANK payment'
    int c = 0;		// if condition para makasulod sa 'CONFIRMATION' sa payment
    int g = 0;		// if condition para makasulod sa 'ORDER SUMMARY'
    int r = 1;		// loop para sa 'WHOLE SYSTEM', kung r=1 mo display ug initial login, kung r>=2 mo display ug 'already logged-in'
    int u = 0;		// kung u=1 kay mo display sa 'REGISTERED' nga account, kung u=2 kay mo display sa 'DEFAULT' nga account
	int x=0;	// loop if ang user mo 'ORDER AGAIN' sa same restaurant
    int y=1;	// loop para sa 'MAIN MENU'
    int z=0;	// if condition para makasulod sa 'RESTAURANTS' nga page
    int select=0;	// input para sa number of 'SELECTED FOOD'
	float select2=0;	// input para sa 'QUANTITY' or number of food nga ganahan. Nganong float? kay 'I.MULTIPLY' man siya sa prices (double/float nga data type)
    int select3=0;	// input para sa choices sa 'PAYMENT METHOD'
	double MF1=0, MF2=0, MF3=0, MF4=0, MF5=0;	// variables sa 'FOOD ITEMS', assigned ug '0' para walay abnormal values inig compute
...
while (r>=1) // WHOLE SYSTEM LOOP
{
    while (y==1)	// MAIN PAGE LOOP
    {
		cout << "\n\n----------------------------------------------------------------------------------------------------------------" << endl;
        cout << "\n\t\t\t\t\t\t FoodMonkey"<< endl;
        ...
        switch (command)
        {
            ...
            cout << "\n\t\t\t\t\t\t FoodMonkey" << endl;
            cout << "\t\t\t\t  Your Number 1 Food Delivery App on the Go!\n\n" << endl;
			Sleep(1000);    // kani nga code mo delay ang program for milliseconds, pwede rani siya wala
			if (r>=2 && loguser=="dev" && logpass=="foodmonkey")	// if condition kung na repeat ang system '2 OR MORE' ug ni choose ang user nga mobalik sa 'MAIN PAGE'
			{
			...
			if (r==1)	// if condition para sa initial login
			{
				cout << "\n\t\t\t\t\t      Log-in to Account\n";
				cout << "\n\t\t\t\t Username: ";
				cin.ignore(1,'\n');
    ...
    if (z==1)	// if condition para makasulod sa 'RESTAURANTS PAGE'
    {
    	char choice;	// local declaration para sa input kung mo order again
		command = 0;
    ...
    if (g==1)   // if condition para makasulod sa 'ORDER SUMMARY'
    {
    Sleep(500);
	double MFtotal = MF1+MF2+MF3+MF4+MF5;   // local variables para sa pagcompute sa total for each restaurant
    ...
    if(z==2)    // if condition para makasulod sa 'MODE OF PAYMENT'
    {
        cout << "\n\n----------------------------------------------------------------------------------------------------------------" << endl;
        Sleep(1500);
        int method;     // input para sa pagpili sa mode of payment
        int proceed;    // INIG DEFEND PALIHUG KO INGON WALA NI FUNCTION, NALIMTAN NI UG TANGTANG
        string oluser;      // variable para sa paginput sa 'ONLINE PAYMENT nga USERNAME' (bisag unsa rani)
        string olpass;      // variable para sa paginput sa 'ONLINE PAYMENT nga PASSWORD' (bisag unsa rani)
        string bcardnum;    // variable para sa paginput sa 'CARD NUMBER'
        string bname;       // variable para sa paginput sa 'NAME' sa cardholder
        string cvv;     // aka Card Verification Code or Card Verification Value, variable para sa paginput sa 'CVV'
    ...
    if (c==1)   // condition para makasulod sa 'CONFIRM PAYMENT'
    {    
        cout << "\n----------------------------------------------------------------------------------------------------------------" << endl;
        char confirm;   // input para sa pag confirm sa payment
        if (u==1)
        {
        ...
        else if (confirm=='N' || confirm=='n')
        {
            ...
            cin >> confirm;     
            cout << "\n";
            if (confirm=='Y' || confirm=='y')   // mobalik sa main page ang program
            {
            ...
            else if (confirm=='N' || confirm=='n')  // mobalik sa order summary then payment
            {
                c=0;
                z=0;
				Sleep(2000);
